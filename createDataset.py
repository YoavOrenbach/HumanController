...
def save_pose_data(camera, key, data_folder="dataset"):
    count = 0
    while count < 100:
        ...
        Image.fromarray(frame).save(data_folder+'/'+key+'/img'+str(count)+'.png')
        cv2.waitKey(1)
        count = count + 1


def collectKeyData(key, camera_port=0, data_folder="dataset"):
    create_data_folder(data_folder)
    cv2.namedWindow("preview")
    camera = cv2.VideoCapture(camera_port, cv2.CAP_DSHOW)
    create_data_folder(data_folder + "/" + key)
    timer(camera)
    save_pose_data(camera, key, data_folder)
    camera.release()
...
def choose_keys(data_folder="dataset"):
    ...
    while key_input != "exit":
        collectKeyData(key_input, data_folder=data_folder)
        key_input = input("please enter the key you want to train or type exit if you are finished: ")
